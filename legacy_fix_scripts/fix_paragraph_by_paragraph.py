...
OUTPUT ONLY THE COMPLETE JAVA METHOD - no markdown, no explanation."""
    ...
    def run_program(self) -> tuple:
        """Run the program and return (success, error_message)"""
        runner_class = f"{self.program_name}Runner"
        result = subprocess.run(
            ["./mvnw", "-q", "-DskipTests", "exec:java", 
             f"-Dexec.mainClass=com.fordcredit.misc1099.batch.runner.{runner_class}"],
            cwd=MISC_1099_DIR,
            capture_output=True,
            text=True,
        ...
        for i, para in enumerate(paragraphs[start_from:end_idx], start=start_from):
            ...
            if self.replace_method(method_name, new_code):
                ...
                if success:
                    print(f"    ✅ Compiled!")
                    # Also verify it runs
                    run_success, run_error = self.run_program()
                    if run_success:
                        print(f"    ✅ Executed successfully!")
                        converted += 1
                    else:
                        print(f"    ⚠️ Compiled but execution failed: {run_error}")
                        print(f"    ⚠️ Continuing anyway (may need test data)...")
                        converted += 1  # Still count as converted since it compiles
                else:
                    ...
                    if self.run_fixer(20):
                        ...
                        if run_success:
                            ...
                            print(f"    ⚠️ Compiled but execution failed: {run_error}")
                            converted += 1  # Still count as converted
                    else:
